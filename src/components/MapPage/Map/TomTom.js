...
export const init_map = (problem, carType) => {
  let url = "";

  if (problem == undefined) {
    url = `/garages`;
  } else if (problem !== "all") {
    url = `/garages/getGarageByServiceTypeAndCarType/${problem}/${carType}`;
  } else {
    url = `/garages/getGarageByCarType/${carType}`;
  }
...
function flyToLocation(obj) {
  map.flyTo(obj);
}

function addMarkerOnClick(e, obj) {
  var marker = new tt.Marker(obj).setLngLat(e.lngLat).addTo(map);

  if (markers.length == 0) locations += "" + e.lngLat.lng + "," + e.lngLat.lat;
...
export function displayPopups(popupOffsets) {
  var i = 0;
  ...
  garagesPopups.map((garagePopup) => {
    ...
    imagesNames.push(
      `http://localhost:8080/garages/${garagePopup.garageID}/profileImage/${random}`
    );
    markersPositions.push(new tt.Marker().setLngLat(position).addTo(map));
  });
